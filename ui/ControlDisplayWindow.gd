...
func _build_ui():
	...
	_add_key("X", "handbrake", Vector2(left_x_staggered + (KEY_SIZE.x + KEY_GAP) * 2, row2_y))
...
func _add_key(lbl: String, action: String, pos: Vector2, raw_key: bool = false, key_code: int = 0) -> ControlDisplayKey:
	var key = ControlDisplayKey.new()
	key.position = pos
	key.size = KEY_SIZE
	key.set_key(lbl, action, raw_key, key_code)
	add_child(key)
